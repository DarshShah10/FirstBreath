...
- After simulation completes, doesn't exit immediately, enters command waiting mode
...
# This fixes the OASIS third-party library reading files without specifying encoding
# ============================================================
import sys
import os
...
if sys.platform == 'win32':
    ...
    def _utf8_open(file, mode='r', buffering=-1, encoding=None, errors=None, 
        ...
        This fixes the issue of third-party libraries (like OASIS) reading files without specifying encoding
        """
...
def disable_oasis_logging():
    ...
    OASIS logs are too verbose (logging each agent's observation and action), we use our own action_logger
    """
...
def init_logging_for_simulation(simulation_dir: str):
    ...
    # Clean old log directory (if exists)
...
class ParallelIPCHandler:
    ...
    Manages two platform environments, handles Interview commands
    """
    ...
    async def handle_interview(self, command_id: str, agent_id: int, prompt: str, platform: str = None) -> bool:
        ...
        Args:
            ...
            platform: Specify platform (optional)
                ...
                - None/not specified: Interview both platforms, return merged results
    ...
    async def handle_batch_interview(self, command_id: str, interviews: List[Dict], platform: str = None) -> bool:
        ...
        twitter_interviews = []
        reddit_interviews = []
        both_platforms_interviews = []  # Needs to interview both platforms
...
def fetch_new_actions_from_db(
    ...
    """
    Fetch new action records from database and supplement complete context information
    
    Args:
        db_path: Database file path
        last_rowid: Last maximum rowid value (use rowid instead of created_at because different platforms have different created_at formats)
    ...
    try:
        ...
        for rowid, user_id, action, info_json in cursor.fetchall():
            ...
            # Simplify action_args, only keep key fields (keep complete content, no truncation)
            simplified_args = {}
...
def _enrich_action_context(
    ...
    """
    Supplement context information for actions (post content, username, etc.)
    ...
    try:
        ...
        # Create comment: supplement commented post information
        ...
        # Context enrichment failure doesn't affect main flow
...
def create_model(config: Dict[str, Any], use_boost: bool = False):
    ...
    # Select which LLM to use based on parameter and config
    ...
    # If no model name in .env, use config as backup
    if not llm_model:
...
async def run_twitter_simulation(
    ...
    # If config doesn't have an agent, use OASIS default name
    ...
    total_actions = 0
    last_rowid = 0  # Track last processed row number in database (use rowid to avoid created_at format differences)
    ...
    for round_num in range(total_rounds):
        ...
        # Get and record actual executed actions from database
...
async def run_reddit_simulation(
    ...
    # Reddit uses boost LLM config (if available, otherwise fallback to general config)
...
async def main():
    ...
    parser.add_argument(
        ...
        action='store_true',
        default=False,
        help='Exit immediately after simulation completes, do not enter command waiting mode'
    )
    
    args = parser.parse_args()
    
    # Create shutdown event at the start of main function to ensure entire program can respond to shutdown signals
...
def setup_signal_handlers(loop=None):
    """
    Setup signal handlers to ensure proper exit when receiving SIGTERM/SIGINT
    
    Persistent simulation scenario: Don't exit after simulation completes, wait for interview commands
    ...
    def signal_handler(signum, frame):
        ...
        if not _cleanup_done:
            _cleanup_done = True
            # Set event to notify asyncio loop to exit (give loop opportunity to clean up)
            if _shutdown_event:
                _shutdown_event.set()
        
        # Don't directly sys.exit(), let asyncio loop exit normally and clean up resources
        # Only force exit if receiving duplicate signals
        else:
            print("Force exit...")
            sys.exit(1)
