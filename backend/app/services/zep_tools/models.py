"""Data models for the Zep Tools service."""
...
@dataclass
class SearchResult:
    """Search result returned by graph queries."""
    facts: List[str]
...
@dataclass
class NodeInfo:
    """Node information retrieved from the Zep graph."""
    uuid: str
    name: str
    labels: List[str]
    summary: str
...
@dataclass
class EdgeInfo:
    """Edge information retrieved from the Zep graph, including temporal metadata."""
    uuid: str
    name: str
    fact: str
...
class InsightForgeResult:
    ...
    Contains retrieval results across multiple sub-queries plus a consolidated analysis.
    """
    query: str
...
class PanoramaResult:
    """
    Breadth search result (PanoramaSearch).

    Contains all related information including expired/invalidated content.
    """
    query: str
    ...
    def to_text(self) -> str:
        ...
        text_parts = [
            f"## Breadth Search Results (Panoramic Future View)",
            f"Query: {self.query}",
            f"\n### Statistics",
        ...
        # Active facts (complete output, untruncated)
        if self.active_facts:
            text_parts.append(f"\n### [Active Facts] (verbatim simulation results)")
            for i, fact in enumerate(self.active_facts, 1):
                text_parts.append(f"{i}. \"{fact}\"")

        # Historical/expired facts (complete output, untruncated)
        if self.historical_facts:
            text_parts.append(f"\n### [Historical/Expired Facts] (evolution record)")
            for i, fact in enumerate(self.historical_facts, 1):
                text_parts.append(f"{i}. \"{fact}\"")

        # Key entities (complete output, untruncated)
        if self.all_nodes:
            text_parts.append(f"\n### [Entities Involved]")
...
class AgentInterview:
    """Result of a single agent interview."""
    agent_name: str
    ...
    key_quotes: List[str] = field(default_factory=list)  # Key quotes extracted from the response
    ...
    def to_text(self) -> str:
        ...
        if self.key_quotes:
            ...
            for quote in self.key_quotes:
                ...
                # Filter lines that contain question number markers ("问题1" through "问题9" in Chinese)
                skip = False
                for d in '123456789':
                ...
                if skip:
                    continue
                # Truncate overly long quotes at a sentence boundary rather than a hard cut
...
class InterviewResult:
    ...
    # Individual agent interview responses
