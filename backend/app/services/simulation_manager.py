...
class SimulationManager:
    ...
    3. Use the LLM to intelligently generate simulation configuration parameters.
    ...
    def _get_simulation_dir(self, simulation_id: str) -> str:
        """Return (and create if needed) the data directory for a simulation."""
    ...
    def _load_simulation_state(self, simulation_id: str) -> Optional[SimulationState]:
        """Load simulation state from disk (or return from cache)."""
    ...
    def prepare_simulation(
        ...
        2. Generate an OASIS Agent Profile for each entity (optional LLM enrichment, parallel).
        3. Use the LLM to intelligently generate simulation configuration parameters
           (time, activity levels, posting frequency, etc.).
        ...
        Args:
            ...
            defined_entity_types: Pre-defined entity types to filter on (optional).
        ...
        try:
            ...
            if filtered.filtered_count == 0:
                ...
                state.error = (
                    "No entities matching the criteria were found. "
                    "Please verify that the graph was built correctly."
                )
                self._save_simulation_state(state)
                return state
            ...
            # Note: run scripts remain in backend/scripts/ and are not copied to the
            # simulation directory. The simulation_runner will invoke them from scripts/.
    ...
    def list_simulations(self, project_id: Optional[str] = None) -> List[SimulationState]:
        """List all simulations, optionally filtered by project ID."""
        simulations = []
    ...
    def get_profiles(
        ...
        """Retrieve Agent Profiles for a simulation."""
        state = self._load_simulation_state(simulation_id)
        if not state:
            raise ValueError(f"Simulation does not exist: {simulation_id}")
    ...
    def get_run_instructions(self, simulation_id: str) -> Dict[str, str]:
        ...
        return {
            ...
            "commands": {
                "twitter": (
                    f"python {scripts_dir}/run_twitter_simulation.py --config {config_path}"
                ),
                "reddit": (
                    f"python {scripts_dir}/run_reddit_simulation.py --config {config_path}"
                ),
                "parallel": (
            ...
            "instructions": (
                ...
                f"   - Twitter only:    python {scripts_dir}/run_twitter_simulation.py"
                f" --config {config_path}\n"
                f"   - Reddit only:     python {scripts_dir}/run_reddit_simulation.py"
                f" --config {config_path}\n"
                f"   - Both in parallel: python {scripts_dir}/run_parallel_simulation.py"
                f" --config {config_path}"
            ),
        }
