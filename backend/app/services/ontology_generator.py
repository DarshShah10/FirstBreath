...
class OntologyGenerator:
    ...
    def generate(
        ...
        Returns:
            Ontology definition dict containing entity_types, edge_types, etc.
        """
    ...
    # Maximum text length (in characters) passed to the LLM (50 000 chars)
    ...
    def _build_user_message(
        ...
        # Truncate if over the limit (only affects LLM input, not graph construction)
        if len(combined_text) > self.MAX_TEXT_LENGTH_FOR_LLM:
            combined_text = combined_text[:self.MAX_TEXT_LENGTH_FOR_LLM]
            combined_text += (
                f"\n\n...(original text is {original_length} chars; "
                f"truncated to the first {self.MAX_TEXT_LENGTH_FOR_LLM} chars for ontology analysis)..."
            )
...
"""
    ...
    def _validate_and_process(self, result: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if fallbacks_to_add:
            ...
            # If adding them would exceed the limit, remove entries from the tail
            # (preserving the more important specific types at the front)
        ...
        # Final defensive cap to ensure limits are never exceeded
    ...
    def generate_python_code(self, ontology: Dict[str, Any]) -> str:
        """
        Convert an ontology definition to Python class code (similar to ontology.py).
        ...
        # Generate relationship type classes
        for edge in ontology.get("edge_types", []):
            name = edge["name"]
            # Convert UPPER_SNAKE_CASE to PascalCase class name
