"""
Report API Routes
Provides simulation report generation, retrieval, chat, and other interfaces
"""
...
def generate_report():
    ...
    This is a time-consuming operation, the endpoint will return task_id immediately.
...
def get_generate_status():
    ...
    try:
        ...
        # If simulation_id is provided, check if completed report already exists
        if simulation_id:
            existing_report = ReportManager.get_report_by_simulation(simulation_id)
            if existing_report and existing_report.status == ReportStatus.COMPLETED:
                return jsonify({
...
def download_report(report_id: str):
    ...
    try:
        ...
        if not os.path.exists(md_path):
            # If MD file doesn't exist, generate a temp file
            import tempfile
...
def get_report_sections(report_id: str):
    ...
    Frontend can poll this endpoint to get generated sections without waiting for entire report
...
def get_agent_log(report_id: str):
    ...
    Real-time retrieval of every step during report generation:
    - Report start, planning start/complete
    - Each section start, tool call, LLM response, complete
    - Report complete or failed
    
    Query Parameters:
        from_line: Start from line number (optional, default 0, for incremental retrieval)
...
def get_console_log(report_id: str):
    ...
    This is different from the structured JSON log returned by agent-log endpoint,
    it's plain text console-style log.
